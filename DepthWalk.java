...
public class DepthWalk {
	...
	public void walking()
	{
        ...
        while((!temp.isTreasure()) && (running == true))
        {
            addNext();
            list.remove(0);
            store.add(temp);
    ...
    public void possibleAdd(Node temp)
    {
        ...
        else if(map.isStandingOnAgent(temp.getY(), temp.getY()))
            robby.checkAlive();
        else if(map.hasStepTreasure(temp.getX(), temp.getY()))
            robby.hasTreasure();
        else if(map.steppingOnEntry(temp.getX(), temp.getY()))
            robby.atEntry();
        else
        	list.add(temp);
    }
